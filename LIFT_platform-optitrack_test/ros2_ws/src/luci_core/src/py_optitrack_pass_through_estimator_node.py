...
from mpclab_msgs.msg import VehicleStateMsg, VehicleActuationMsg
...
class OptiTrackPassThroughEstimatorNode(MPClabNode):
    ...
    def step(self):
        ...
        stamp = now.to_msg()
        if self.publish_as_odom:
            est_state_msg = Odometry()
            est_state_msg.header.stamp = stamp
            est_state_msg.pose.pose.position.x = self.est_state.x.x
            est_state_msg.pose.pose.position.y = self.est_state.x.y
            est_state_msg.pose.pose.position.z = self.est_state.x.z
            est_state_msg.pose.pose.orientation.x = self.est_state.q.qi
            est_state_msg.pose.pose.orientation.y = self.est_state.q.qj
            est_state_msg.pose.pose.orientation.z = self.est_state.q.qk
            est_state_msg.pose.pose.orientation.w = self.est_state.q.qr
            est_state_msg.twist.twist.linear.x = self.est_state.v.v_long
            est_state_msg.twist.twist.linear.y = self.est_state.v.v_tran
            est_state_msg.twist.twist.linear.z = self.est_state.v.v_n
            est_state_msg.twist.twist.angular.x = self.est_state.w.w_phi
            est_state_msg.twist.twist.angular.y = self.est_state.w.w_theta
            est_state_msg.twist.twist.angular.z = self.est_state.w.w_psi
        else:
            est_state_msg = self.populate_msg(VehicleStateMsg(), self.est_state)
            est_state_msg.header.stamp = stamp
            est_state_msg.timing.step_start_time = t
            est_state_msg.timing.source_time = self.posevel_meas.t
            tf = self.clock.now().nanoseconds/1E9
            est_state_msg.timing.publish_time = tf
            est_state_msg.timing.step_execution_time = tf - t
