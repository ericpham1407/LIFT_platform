...
import optitrack_client.DataDescriptions as DataDescriptions
import optitrack_client.MoCapData as MoCapData
...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        self.__application_name = "OptiTrack"
        ...
        self.queue_size = 0
        self.mocap_data_queue = mp.Queue()
...
#Unpack Mocap Data Functions
    ...
    def __data_thread_function(self, in_socket, in_queue, stop, gprint_level):
        message_id_dict={}
        data=bytearray(0)
        # 64k buffer size
        recv_buffer_size=64*1024
        ...
        while not stop():
            ...
            try:
                data, addr = in_socket.recvfrom( recv_buffer_size )
                # print(f'Receive time: {time.time()-t}')
            except socket.error as msg:
                if not stop():
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message(data, in_queue, print_level)

                data=bytearray(0)
            # print(f'Data thread time: {time.time()-t}')
        return 0

    def __process_message(self, data : bytes, queue, print_level=0):
        #return message ID
    ...
    def get_mocap_data(self):
        if not self.mocap_data_queue.empty():
            # if self.mocap_data_queue.qsize() > self.queue_size:
            #     self.queue_size = self.mocap_data_queue.qsize()
    ...
    def run( self ):
        ...
        self.data_thread = mp.Process(target=self.__data_thread_function, args = (self.data_socket, self.mocap_data_queue, lambda : self.stop_threads, lambda : self.print_level, ))
...
if __name__ == "__main__":
    ...
    # frame_data = dict()
    # def frame_listener(data):
    #     frame_data = data
    # streaming_client.new_frame_listener = frame_listener
    ...
    last_time = 0
    while True:
        mocap_data = streaming_client.get_mocap_data()
        if mocap_data:
            delta_t = mocap_data.suffix_data.timestamp - last_time
            last_time = copy.copy(mocap_data.suffix_data.timestamp)
            print(delta_t)
